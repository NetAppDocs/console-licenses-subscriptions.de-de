...
NOTE: Für AWS kann es sein, dass Ihr Konto mehr als ein NetApp Marketplace-Abonnement zur Verfügung hat. Wenn nur das eigenständige Cloud Volumes ONTAP Abonnement verknüpft wird, ist die Bereitstellung von Cloud Volumes ONTAP mit Essentials möglich, aber für die Konfiguration von Backup und Recovery ist ein anderes Abonnement erforderlich. Zur Auswahl des Abonnements, das für die Kapazitätsabrechnung von Cloud Volumes ONTAP verwendet wird (einschließlich der Änderung der Abrechnung für bestehende Systeme), befindet sich die Option unter *Administration > Licenses and subscriptions > Billing preferences*.
