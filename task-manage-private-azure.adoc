...
summary: Akzeptieren und abonnieren Sie ein privates Marketplace-Angebot für die NetApp Console im Azure-Portal.  Konfigurieren Sie Azure-Anmeldeinformationen in der Konsole. 
---
...
Wenn Sie das Angebot mehrfach annehmen und aktivieren, kann dies zu einer doppelten Abbuchung führen.

====
.Schritte
. Wählen Sie den von Ihrem NetApp Vertriebsteam bereitgestellten Link zum privaten Angebot aus und melden Sie sich an. Falls kein Link verfügbar ist, melden Sie sich direkt im Azure-Portal an.
. Suchen oder wählen Sie *Marketplace* aus den Azure-Diensten aus.
. Wählen Sie auf der Marketplace-Seite unter dem Abschnitt *Verwaltung* die Option *Private Angebotverwaltung* aus.
. Wählen Sie *Ansehen + annehmen*, um die Details des privaten Angebots einzusehen.
+
...
. Bestätigen Sie und wählen Sie *Privates Angebot annehmen*.
. Wählen Sie *Kaufen*, um den Kaufvorgang abzuschließen.
+
[NOTE]
====
...
. Wählen Sie *Überprüfen und abonnieren*, bestätigen Sie die Angaben und wählen Sie dann *Abonnieren*.
. Sobald das Abonnement erfolgreich erstellt wurde, wählen Sie *Konto jetzt konfigurieren*. Sie werden dann zur NetApp Console weitergeleitet, um die Konfiguration abzuschließen.
+
[NOTE]
====
Die Azure-seitige Automatisierung kann zwischen 30 Sekunden und 5 Minuten dauern. Wenn die Seite geschlossen wurde, bevor Sie *Konto jetzt konfigurieren* ausgewählt haben, kehren Sie im Azure-Portal zum Dienst zurück. Das Abonnement wird als *Konfiguration ausstehend* angezeigt. Wählen Sie es aus, um die Konfiguration fortzusetzen.
...
.. Wählen Sie die Console-*Organizations* aus, die Zugriff auf dieses Abonnement haben sollen.
.. Optional können Sie die Option *Vorhandenes Abonnement ersetzen* aktivieren, um ein bestehendes Marketplace-Abonnement in einer Console-Organisation automatisch zu ersetzen.
+
[NOTE]
====
*** Diese Austauschoption ist auf eine einzelne Console-Organisation beschränkt.
...
Weitere Informationen zu Marktplatz-Abonnements und zur Abrechnung von Direktlizenzen finden Sie unter link:concept-billing-preferences.html["Erfahren Sie mehr über die Abrechnungseinstellungen für Lizenzen und Abonnements."].
